...
class TimelineMediaPreviewViewModel: TimelineMediaPreviewViewModelType {
    ...
    private func updateCurrentItem(_ previewItem: TimelineMediaPreviewItem) async {
        ...
        if case let .media(mediaItem) = previewItem {
            ...
            if mediaItem.fileHandle == nil, let source = mediaItem.mediaSource {
                ...
                // second time, doubling the traffic. Started before the neighbours' loads so it
                // keeps the head download slot, and registered so they and the placeholder grace see it.
                if let inFlight = preloads[mediaItem.id] {
                    ...
                    let task = Task { [weak self, mediaProvider] in
                        ...
                        return await mediaProvider.loadFileFromSource(source, filename: mediaItem.filename)
                    }
    ...
    private func preparePlaceholder(for item: TimelineMediaPreviewItem.Media) async {
        ...
        guard let job = await placeholderJob(for: item, thumbnailTimeout: Self.thumbnailTimeoutOnDisplay) else {
        ...
        let written = await Task.detached(priority: .userInitiated) {
            Self.writePlaceholder(job.thumbnail, size: job.size, to: job.url, in: directory)
        }.value
        guard written, item.fileHandle == nil, item.placeholderURL == nil else { return }
        item.placeholderURL = job.url
        MXLog.info("Media viewer: placeholder for \(item.id) \(Int(job.size.width))x\(Int(job.size.height))")
    ...
    private func startInitialLoad() {
        ...
        initialPresentationGate = Task { [weak self] in
            ...
            let placeholder = Task { [weak self] () -> (URL, CGSize)? in
                guard let self, let job = await placeholderJob(for: item) else { return nil }
                let directory = placeholderDirectory
                let written = await Task.detached(priority: .userInitiated) {
                    Self.writePlaceholder(job.thumbnail, size: job.size, to: job.url, in: directory)
                }.value
                return written ? (job.url, job.size) : nil
            }
    ...
    private func placeholderJob(for item: TimelineMediaPreviewItem.Media,
                                thumbnailTimeout: Duration = TimelineMediaPreviewViewModel.thumbnailTimeout) async -> PlaceholderJob? {
        ...
        return PlaceholderJob(thumbnail: thumbnail, size: size, url: placeholderDirectory.appendingPathComponent("\(UUID().uuidString).jpg"))
    }
